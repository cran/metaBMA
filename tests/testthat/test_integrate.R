...
test_that("check old prior labels with logml='stan'/'integrate'", {
  ...
  suppressWarnings({
    mr1 <- meta_random(logOR, SE, study, towels, summarize = "int",rel.tol = .01, logml_iter = 100, iter = 1000,
                       d = prior("t", c(0, .1, 5), lower=0),
                       tau = prior("norm", c(0,.3), lower = 0))
    mr2 <- meta_random(logOR, SE, study, towels, summarize = "stan", iter = 1000, logml_iter = 1000,
...
test_that("extreme priors/misspecified models still provide correct results", {
  ...
  mr <- meta_random(logOR, SE, study, towels, rel.tol = .1, iter = 1000, logml_iter = 10,
                    d = prior("norm", c(mean = 0.2, sd = .01)))
