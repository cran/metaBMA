...
test_that("Gronau (2017): power pose analysis (reported) are correct ", {
  ...
  suppressWarnings( # divergent transitions
    m_testing <- meta_bma(
      y = effectSize, SE = SE, d = priorEStesting, data = power_pose,
  ...
  m_estimation <- meta_bma(
    y = effectSize, SE = SE, data = power_pose, d = priorESestimation,
    tau = priorTau, iter = 2000, rel.tol = .1,
    logml_iter = 10, summarize = "int"
  )
  expect_equal(m_estimation$estimates["averaged", c(4, 6:7)],
    c("50%" = 0.22, "hpd95_lower" = 0.09, "hpd95_upper" = 0.34),
    tolerance = .03
  )
})
...
test_that("Gronau (2017): power pose analysis with informed t prior ", {
  ...
  # conduct analyses
  suppressWarnings(m_testing <- meta_bma(
    y = effectSize, SE = SE, d = priorEStesting,
    data = power_pose, tau = priorTau, iter = 10
  ))
  ...
  expect_equal(m_estimation$estimates["averaged", c(4, 6:7)],
    c("50%" = 0.26, "hpd95_lower" = 0.14, "hpd95_upper" = 0.37),
    tolerance = .01
  )
})
...
test_that("Gronau (2017): power pose analysis (only unfamiliar with default prior)", {
  ...
  priorEStesting <- prior(
    family = "t", param = c(location = 0, scale = 1 / sqrt(2), nu = 1),
    lower = 0, label = "d"
  )
  ...
  suppressWarnings(m_testing <- meta_bma(
    y = effectSize, SE = SE, d = priorEStesting,
    data = power_pose_unfamiliar, tau = priorTau, iter = 10
  ))
  ...
  m_estimation <- meta_bma(
    y = effectSize, SE = SE, data = power_pose_unfamiliar,
    d = priorESestimation,
    tau = priorTau, iter = 1000, summarize = "int"
  )
  expect_equal(m_estimation$estimates["averaged", c(4, 6:7)],
    c("50%" = .18, "hpd95_lower" = .03, "hpd95_upper" = .33),
    tolerance = .01
  )
})
...
test_that("Gronau (2017): power pose analysis (only unfamiliar and informed t prior) ", {
  ...
  expect_equal(m_estimation$estimates["averaged", c(4, 6:7)],
    c("50%" = .23, "hpd95_lower" = .10, "hpd95_upper" = .36),
    tolerance = .02
  )
})
